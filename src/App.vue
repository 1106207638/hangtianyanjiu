<template>
  <div id="app" :class="{ active: isActive }" >
    <Headers ref="headers" :isSearch="isSearch" :isZoom="isZoom" :headers="isSmall" :Logindialog="isLogin" @yess="yes" v-if="isShow" @loginOut="loginout"/>
...
export default {
  ...
  watch:{
    $route:{
      handler(val,oldval){
        console.log(val)
        if(val.name == 'hello'|| val.name=="Nav"||val.name=="Translate") {
          this.isShow = false
        }else {
          this.isShow = true
        }
